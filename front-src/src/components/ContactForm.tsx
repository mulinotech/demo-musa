...
interface ContactFormProps {
  /** Devolve a promessa do envio: a tela de sucesso so aparece se ela cumprir. */
...
export default function ContactForm({ onLeadCaptured, selectedTreatmentName, onClearSelectedTreatment }: ContactFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    /* A espera aqui e o envio de verdade, e nao mais um `setTimeout` de 1,2s
     * enfeitando um envio que seguia sozinho. A diferenca aparece quando da
     * errado: antes a tela de "recebemos seu contato" ja estava no ar quando o
     * servidor recusava, e a pessoa ia embora pre-agendada de mentira. */
    try {
      ...
      setErrorMsg(
        "Não conseguimos registrar seu pré-agendamento agora. Tente novamente em instantes " +
        "ou fale com a gente pelo WhatsApp — seus dados NÃO foram enviados."
      );
    } finally {
