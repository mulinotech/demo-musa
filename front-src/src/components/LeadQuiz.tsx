...
interface LeadQuizProps {
  /** Devolve a promessa do envio: o resultado so aparece se ela cumprir. */
  onLeadCaptured: (newLead: Lead) => Promise<void>;
...
export default function LeadQuiz({ onLeadCaptured, presetTreatmentName, onClearPresetName }: LeadQuizProps) {
  ...
  const handleFormSubmit = async (e: FormEvent) => {
    ...
    /* O resultado do quiz so aparece DEPOIS que o servidor gravou o contato.
     * Antes era um `setTimeout` de 1,2s: o protocolo era revelado e o envio
     * seguia sozinho, de modo que uma recusa do servidor nao mudava nada na
     * tela -- a pessoa saia achando que a clinica tinha os dados dela. */
    try {
      await onLeadCaptured(newLead);
      setShowResult(true);
      if (onClearPresetName) onClearPresetName();
    } catch (err) {
      setErrorMsg(
        "Não conseguimos registrar seus dados agora. Tente novamente em instantes " +
        "ou fale com a gente pelo WhatsApp — eles NÃO foram enviados."
      );
    } finally {
          ...
          {step === 4 && !showResult && (
            <div className="space-y-6 animate-slide-in-from-right">
              ...
              <form onSubmit={handleFormSubmit} className="space-y-4 max-w-md mx-auto pt-2">
                ...
                {errorMsg && (
                  <p className="text-xs text-red-600 bg-red-50 border border-red-200 rounded-sm px-3 py-2 leading-relaxed">
                    {errorMsg}
                  </p>
                )}
