...
import { DINHEIRO as PAPEIS_DO_DINHEIRO, VE_VISAO_GERAL, telaInicial } from "./lib/papeis.mjs";
...
/* Para onde vai quem não tem o que ver na Visão Geral, e para onde vai um
 * caminho desconhecido dentro do console. Componente, e não uma constante,
 * porque o papel só existe depois que a pessoa entrou. */
function ParaOInicio() {
  return <Navigate to={telaInicial(papelDoToken())} replace />;
}

function IndiceDoConsole() {
  return VE_VISAO_GERAL.includes(papelDoToken()) ? <VisaoGeral /> : <ParaOInicio />;
}
    ...
    <>
      ...
      <Routes>
        ...
        <Route path="/crm" element={<ConsoleProtegido />}>
          <Route index element={<IndiceDoConsole />} />
          ...
          <Route
            path="financeiro"
            element={
              <RotaProtegida papeis={PAPEIS_DO_DINHEIRO}>
                <Financeiro />
              </RotaProtegida>
            }
          />
          <Route
            path="precificacao"
            element={
              <RotaProtegida papeis={["admin", "gerente", "gerente_admin", "financeiro"]}>
                <Precificacao />
              </RotaProtegida>
            }
          />
          ...
          />
          {/* Caminho desconhecido dentro do CRM volta para a tela inicial de
              QUEM ESTÁ LOGADO -- mandar todo mundo para /crm devolveria a
              secretária e o contador para a tela que eles não alcançam. */}
