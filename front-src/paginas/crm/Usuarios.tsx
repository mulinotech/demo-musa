...
/* A LISTA SAIU DAQUI (M6.7).
 *
 * Ela morava neste arquivo e a barra lateral tinha a própria cópia, com quatro
 * papéis cada uma. Agora são nove, e duas listas com nove itens divergem — a
 * pergunta não é se, é quando. `src/lib/papeis.mjs` é a única, e a suíte do
 * servidor confere que ela bate com `PAPEIS_VALIDOS` em routes/users.js e com
 * o ENUM da migration 043. */
import { PAPEIS, rotuloDoPapel as rotuloPapel } from "../../lib/papeis.mjs";

interface Papel { valor: string; rotulo: string; ajuda: string; legado?: boolean }
        ...
        <div className="bg-white border border-brand-gold/15 rounded-2xl p-5 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              {/* O papel LEGADO não é oferecido para acesso novo: "Gerente
                  (modelo antigo)" só existe para não mexer em quem já tem. */}
              <select className={campo} value={novo.role} onChange={(e) => setNovo({ ...novo, role: e.target.value })}>
                {PAPEIS.filter((p: Papel) => !p.legado).map((p: Papel) => (
              ...
              <p className="text-[10px] text-brand-brown/55 mt-1">
                {PAPEIS.find((p: Papel) => p.valor === novo.role)?.ajuda}
              </p>
            </div>
          </div>
                  ...
                  return (
                    <tr key={u.id} className="border-t border-brand-gold/10 align-top">
                      ...
                      <td className="px-4 py-3">
                        ...
                        >
                          {/* Aqui o legado aparece SÓ para quem já o tem: some
                              da lista assim que a clínica escolher um dos dois
                              novos, e não reaparece por engano. */}
                          {PAPEIS.filter((p: Papel) => !p.legado || p.valor === u.role).map((p: Papel) => (
                            <option key={p.valor} value={p.valor} title={p.ajuda}>
                              {p.rotulo}
                            </option>
                          ))}
                        </select>
